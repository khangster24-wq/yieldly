...
- Adjust for the student's self-reported stats vs. the school's typical accepted range if you collect them during onboarding (GPA/test score relative position). If the student doesn't provide stats, fall back to admission-rate-only risk with a visible note that it's unpersonalized.
